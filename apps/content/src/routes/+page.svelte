...
<svelte:head>
  <title>Контент — Automotive Portal</title>
</svelte:head>

<PageHeader title="Контент" subtitle={total > 0 ? `${total} цитат` : ''} backHref="/" backLabel="На главную" />
...
<div class="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-4">
  ...
  {#if data.renderMode === 'spa' && spaLoading}
    <Loader text="Загружаем цитаты..." />
  {:else if spaError}
    <ErrorState message={spaError} onRetry={fetchQuotes} />
  {:else if quotes.length === 0}
    <EmptyState title="Нет цитат" description="Не удалось загрузить цитаты" icon="🔍" />
  {:else}
    <p class="text-xs text-gray-400 px-1">{quotes.length} цитат</p>
