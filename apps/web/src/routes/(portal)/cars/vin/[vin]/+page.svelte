<script lang="ts">
  ...
  import { PageHeader, Card, Input, ErrorState, EmptyState, Loader } from '@repo/shared/ui';
  ...
  async function fetchVin(vin: string) {
    ...
    try {
      ...
      const res = await fetch(`/cars/vin/${encodeURIComponent(vin)}`);
      if (!res.ok) {
...
<Seo
  title={vehicle ? `VIN ${data.vin} — ${vehicle.make} ${vehicle.model}` : `VIN Декодер — ${data.vin}`}
  description={vehicle
    ? `Информация об автомобиле ${vehicle.make} ${vehicle.model} ${vehicle.year} по VIN-коду ${data.vin}`
    : `Информация об автомобиле по VIN-коду ${data.vin}`}
  canonical="https://example.com/cars/vin/{data.vin}"
  jsonLd={vehicle ? {
...
<PageHeader title="VIN Декодер" subtitle="Узнайте всё об автомобиле" backHref="/cars" backLabel="Назад" />
      ...
      <div class="flex items-center gap-3 mb-4 pb-4 border-b border-gray-50">
        ...
        <div>
          <h2 class="text-lg font-bold text-gray-900">
            <span data-testid="vehicle-make">{vehicle.make}</span>
            <span data-testid="vehicle-model">{vehicle.model}</span>
          </h2>
          <p class="text-sm text-gray-500"><span data-testid="vehicle-year">{vehicle.year}</span> · VIN: {vehicle.vin}</p>
        </div>
      </div>
