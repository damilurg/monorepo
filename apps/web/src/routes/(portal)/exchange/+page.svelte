<script lang="ts">
  ...
  async function fetchRates(base: string) {
    ...
    spaError = '';
    try {
      const res = await fetch(`/exchange?base=${base}`);
  ...
  onMount(() => {
    if (data.renderMode === 'spa') {
      fetchRates(data.base);
    }
  });

  // Unified access: SSR uses data.rates, SPA uses spaRates
...
<PageHeader
  title={t.title}
  subtitle={rates ? `Курсы на ${rates.date}` : t.loading}
  backHref="/"
  backLabel={t.back}
/>
  ...
  {:else if rates}
    ...
    <Card>
      <div class="flex flex-col gap-3">
        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">{t.baseCurrency}</p>
        <div class="flex flex-wrap gap-2">
          {#each POPULAR_CURRENCIES as cur (cur.code)}
            {#if availableCurrencies.includes(cur.code) || data.renderMode === 'ssr'}
              {#if data.renderMode === 'ssr'}
                <a
                  href="/exchange?base={cur.code}"
              ...
              {:else}
                <button
                  onclick={() => handleBaseChange(cur.code)}
                  class="px-3 py-1.5 rounded-full text-sm font-medium transition-colors
                    {currentBase === cur.code ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
                >
                  {cur.code}
    ...
    <Card>
      <div class="flex flex-col gap-4">
        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">{t.converter}</p>
        <Input label={t.amount} type="number" bind:value={amount} placeholder="1" />

        <div class="flex gap-3 items-end">
          <div class="flex-1">
            <label class="text-sm font-medium text-gray-700 block mb-1.5">{t.from}</label>
          ...
          <button onclick={swapCurrencies} class="mb-0.5 w-10 h-10 flex items-center justify-center rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors flex-shrink-0" aria-label={t.swap}>⇄</button>

          <div class="flex-1">
            <label class="text-sm font-medium text-gray-700 block mb-1.5">{t.to}</label>
    ...
    <!-- All rates -->
    <Card>
      <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">{t.allRates} {rates.base}</p>
