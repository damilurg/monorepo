<script lang="ts">
  ...
  async function fetchRates(base: string) {
    ...
    spaError = '';
    try {
      const res = await fetch(`/exchange/base/${base.toLowerCase()}`);
  ...
  function handleBaseChange(base: string) {
    goto(`/exchange/base/${base.toLowerCase()}`);
  }
</script>

<Seo
  title="{currentBase} — Курсы валют"
  description="Актуальные курсы валют на основе {currentBase}. Данные от Европейского центрального банка."
  canonical="https://example.com/exchange/base/{data.base}"
  jsonLd={{
    '@context': 'https://schema.org',
    '@type': 'FinancialService',
    name: `Курсы валют — ${currentBase}`,
    description: `Актуальные курсы валют на основе ${currentBase}`,
    url: `https://example.com/exchange/base/${data.base}`,
  }}
/>
  ...
  {:else if rates}
    ...
    <Card>
      <div class="flex flex-col gap-3">
        <p data-testid="base-currency" class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Базовая валюта</p>
        <div class="flex flex-wrap gap-2">
          {#each POPULAR_CURRENCIES as cur (cur.code)}
            {#if availableCurrencies.includes(cur.code) || data.renderMode === 'ssr'}
              <a
                data-testid="currency-btn-{cur.code.toLowerCase()}"
                href="/exchange/base/{cur.code.toLowerCase()}"
    ...
    <Card>
      <div class="flex flex-col gap-4">
        ...
        <Input label="Сумма" type="number" bind:value={amount} placeholder="1" data-testid="converter-amount" />
        ...
        {#if result}
          <div data-testid="converter-result" class="bg-blue-50 rounded-xl p-4 text-center">
    ...
    <Card>
      ...
      <div data-testid="rates-list" class="flex flex-col divide-y divide-gray-50">
