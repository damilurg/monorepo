<script lang="ts">
  ...
  const T = {
    ru: {
      ...
      search: 'Найти',
      markers: 'Маркеры',
      disabled: 'Раздел временно недоступен',
      back: 'На главную',
    ...
    en: {
      ...
      search: 'Search',
      markers: 'Markers',
      disabled: 'Section temporarily unavailable',
      back: 'Home',
  ...
  // Internal marker type that stores Leaflet marker ref
  ...
  let locationName = $state(
    data.disabled ? '' : data.renderMode === 'ssr' ? data.locationName : T[langStore.current].mapLoading
  );
  ...
  async function searchLocation() {
    ...
    try {
      const res = await fetch(`/maps?q=${encodeURIComponent(searchQuery.trim())}`);
      ...
      if (leafletMap && leafletLib) {
        ...
        goto(`/maps/${dto.coordinates.lat.toFixed(6)}/${dto.coordinates.lon.toFixed(6)}`, {
          replaceState: true,
          noScroll: true,
        });
      }
  ...
  function flyToCity(city: { lat: number; lon: number; name: string; country: string }) {
    goto(`/maps/${city.lat}/${city.lon}`);
  }

  function formatTime(iso: string): string {
    return new Date(iso).toLocaleTimeString(langStore.current === 'ru' ? 'ru-RU' : 'en-US', { hour: '2-digit', minute: '2-digit' });
  }
...
{#if data.disabled}
  <div class="mx-auto max-w-2xl px-4 py-20 text-center">
    <p class="text-gray-500 text-lg">{t.disabled}</p>
    <a href="/" class="mt-4 inline-block text-blue-600 hover:underline">{t.back}</a>
  </div>
{:else}
  <Seo
    title="Карты — {locationName}"
    description="Интерактивные карты OpenStreetMap. Поиск мест, добавление маркеров."
    canonical="https://example.com/maps"
    jsonLd={{
      '@context': 'https://schema.org',
      '@type': 'WebPage',
      name: `Карты — ${locationName}`,
      description: 'Интерактивные карты OpenStreetMap',
